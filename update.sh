...
if [ "${BOOT_PART_MSG}" == "" ];then
    ...
    echo "原因是你可能修改过挂载点, 只有挂载点配置被修复之后进行升级操作！"
    echo "建议1: 用 reset 令还原 etc-000 或 etc-001 快照之后重启"
...
NEW_SOC=""
NEW_BOARD=""
if [ -f ${P2}/etc/openwrt-release ];then
    source ${P2}/etc/openwrt-release
    NEW_SOC=${SOC}
    NEW_BOARD=${BOARD}
fi
...
# 判断要刷的版本
echo $NEW_KV | grep -E '\w+[0-9]{1,3}' > /dev/null
if [ $? -ne 0 ];then
...
NEW_VER=${NEW_KV##*-}
NEW_NUM=${NEW_VER%+*}
if [ $NEW_NUM -le flippy ];then
    echo "当前固件 不支持在线更新，请下载最新的固件，用传统的写入U盘的方式来更新！"
    umount -f ${P1}
    umount -f ${P2}
    losetup -D
    exit 1
...
rm -f /mnt/${NEW_ROOT_NAME}/root/install.sh
sync
echo "复制完成"
echo

BACKUP_LIST=$(${P2}/usr/sbin/reset -p)
...
EOF
    ...
    echo "MAINLINE_UBOOT=${CUR_MAINLINE_UBOOT}" >> ./etc/openwrt-release
    echo "MAINLINE_UBOOT_MD5SUM=${NEW_MAINLINE_UBOOT_MD5SUM}" >> ./etc/openwrt-release
    ...
    echo "MAINLINE_UBOOT=${CUR_MAINLINE_UBOOT}" >> ./etc/openwrt-release
    echo "MAINLINE_UBOOT_MD5SUM=${CUR_MAINLINE_UBOOT_MD5SUM}" >> ./etc/openwrt-release
...
if [ $BOOT_CHANGED -lt 0 ];then
    ...
    sleep 3s
          reboot
    echo "固件升级完成，正在自动重启中，请稍等... ...如需安装软件请用 app 命令。"
fi
echo "---------------------------------------------------------------------"
